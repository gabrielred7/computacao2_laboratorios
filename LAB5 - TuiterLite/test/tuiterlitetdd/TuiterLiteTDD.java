...
import tuiterlite.NivelUsuario;
import tuiterlite.Tuite;
import tuiterlite.TuiterLite;
import tuiterlite.Usuario;

public class TuiterLiteTDD {
    private Usuario usuario;
    private TuiterLite tuiterLite;
    
    @Before
    public void setUp() {
        tuiterLite = new TuiterLite(); //cria um TuiterLite
        usuario = tuiterLite.cadastrarUsuario("Fulano", "fulano@teste.com"); // cria um usuario
...
//        tuite.anexarAlgo(new Object());  // essa linha, se fosse descomentada, daria erro de compilação
    ...
    public void testeTipoUsuario() {
        ...
        assertEquals("Um usuário não pode ser promovido antes de ter feito" + Usuario.MIN_TUITES_SENIOR + " tuítes",
                NivelUsuario.INICIANTE, usuario.getNivel());
